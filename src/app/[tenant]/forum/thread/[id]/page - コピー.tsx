...
const bestOpinionsByIssue = useMemo(() => {
  return groupedByOpinion.map((group) => {
    const scored = group.opinions.map((op) => {
      ...
      const trustLabel =
        authorTrustMap[op.opinion.author_key ?? ""]?.label;
          ...
          >
            ...
            <div
              style={{
                ...
                fontSize: 14,
                background:
                  structureType === "対立あり（意見 vs 反論が衝突中）"
                    ? "#ffebee"
                    : structureType === "意見集約中"
                    ? "#e3f2fd"
                    : structureType === "整理・解説フェーズ"
                    ? "#f3e5f5"
                    : "#f0f0f0",
              }}
            >
...
{feedbackSummary && (
  ...
  >
    ...
    <div style={{ fontSize: 13, lineHeight: 1.8, color: "#444" }}>
      <div>言葉がわからん：{feedbackSummary.term_unknown}</div>
      <div>前提がわからん：{feedbackSummary.premise_unknown}</div>
      <div>結論がわからん：{feedbackSummary.conclusion_unknown}</div>
      <div>根拠がわからん：{feedbackSummary.evidence_unknown}</div>
      <div>反対意見がわからん：{feedbackSummary.counterargument_unknown}</div>
    </div>
  </div>
)}
            {!summary ? (
              ...
              <>
                ...
                >
                  <span style={badgeStyle}>総投稿 {summary.counts.total}</span>
                  <span style={{ ...badgeStyle, background: "#e3f2fd" }}>
                    論点 {summary.counts.issue_raise}
                  </span>
                  <span style={{ ...badgeStyle, background: "#e8f5e9" }}>
                    意見 {summary.counts.opinion}
                  </span>
                  <span style={{ ...badgeStyle, background: "#ffebee" }}>
                    反論 {summary.counts.rebuttal}
                  </span>
                  <span style={{ ...badgeStyle, background: "#fff8e1" }}>
                    補足 {summary.counts.supplement}
                  </span>
                  <span style={{ ...badgeStyle, background: "#f3e5f5" }}>
                    解説 {summary.counts.explanation}
                  </span>
                </div>
                ...
                {conflicts.length > 0 && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    {conflicts.map((pair, i) => (
                      ...
                      >
                        <div style={{ fontWeight: 600 }}>意見</div>
                        <div>{pair.opinion}</div>

                        <div style={{ marginTop: 6, fontWeight: 600 }}>反論</div>
                        <div>{pair.rebuttal}</div>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div style={{ marginBottom: 18 }}>
                  <div style={sectionTitle}>AIによる要約</div>
                  <div style={sectionBody}>{summary.summary_text}</div>
                </div>

                {summary.key_points.issues.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={sectionTitle}>主な論点</div>
                    <ul style={listStyle}>
                      {summary.key_points.issues.map((item, i) => (
                        <li key={i}>{item}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {summary.key_points.opinions.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={sectionTitle}>主な意見</div>
                    <ul style={listStyle}>
                      {summary.key_points.opinions.map((item, i) => (
                        <li key={i}>{item}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {summary.key_points.rebuttals.length > 0 && (
                  <div>
                    <div style={sectionTitle}>主な反論</div>
                    <ul style={listStyle}>
                      {summary.key_points.rebuttals.map((item, i) => (
                        <li key={i}>{item}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {summary.key_points.supplements.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <div style={sectionTitle}>補足</div>
                    <ul style={listStyle}>
                      {summary.key_points.supplements.map((item, i) => (
                        <li key={i}>{item}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {summary.key_points.explanations.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <div style={sectionTitle}>解説</div>
                    <ul style={listStyle}>
                      {summary.key_points.explanations.map((item, i) => (
                        <li key={i}>{item}</li>
                      ))}
                    </ul>
                  </div>
          ...
          >
            ...
            {bestOpinionsByIssue.some((group) => group.best) && (
              <div style={{ marginBottom: 24 }}>
                ...
                {bestOpinionsByIssue.map((group, i) => {
                  if (!group.best) return null;

                  return (
                    <div
                      key={group.issue?.id ?? `best-${i}`}
                      style={{
                    ...
                    >
                      ...
                      >
                        論点: {group.issue?.content ?? "（論点なし）"}
                      </div>

                      <div style={{ fontWeight: "bold", marginBottom: 6 }}>
                        {group.best.opinion.content}
                      </div>

<div style={{ fontSize: 12, color: "#555" }}>
  元スコア: {group.best.opinion.logic_score ?? "未評価"} ／
  trust: {group.best.trustLabel} ／
  反論数: {group.best.rebuttalCount} ／
  実効スコア: {group.best.effectiveScore}
</div>
                      {group.best.rebuttalCount === 0 && (
                        <div
                          style={{
                            fontSize: 12,
                            color: "#2e7d32",
                ...
                return (
                  <div key={i} style={{ marginBottom: 10 }}>
                    <div style={{ fontWeight: 700 }}>
                      {group.issue?.content ?? "（論点なし）"}
                    </div>

{ranked.map((r, idx) => (
  <div key={idx} style={{ fontSize: 13 }}>
    {idx + 1}位：{r.score} [trust:{r.trustLabel}]
  </div>
))}
                  </div>
                );
...
{unresolvedIssues.length > 0 && (
  ...
  >
    ...
    {unresolvedIssues.map((item, i) => (
      <div key={i} style={{ marginBottom: 12 }}>
        <div style={{ fontWeight: 700, marginBottom: 4 }}>
          {item.issue}
        </div>
<div style={{ fontSize: 13 }}>
  1位：{item.first.score} [trust:{item.first.trustLabel}] / {item.first.content}
</div>
<div style={{ fontSize: 13 }}>
  2位：{item.second.score} [trust:{item.second.trustLabel}] / {item.second.content}
</div>

<div
  style={{
    fontSize: 12,
    color: item.diff < 5 ? "#b00020" : "#666",
    marginTop: 4,
    fontWeight: item.diff < 5 ? 700 : 500,
  }}
>
  {item.diff < 5
    ? `判定：かなり拮抗（差 ${item.diff} 点）`
    : `判定：未決着（差 ${item.diff} 点）`}
</div>

{item.first.trustLabel !== item.second.trustLabel && (
  <div
    style={{
      fontSize: 12,
      color: "#1565c0",
  ...
  >
    trustは {item.first.trustLabel} vs {item.second.trustLabel}
  </div>
)}
...
<div style={{ fontWeight: 700, marginBottom: 6 }}>
  ...
  >
    ...
    {authorTrustMap[op.opinion.author_key ?? ""]?.label ?? "-"}
                            ...
                            <div style={{ marginBottom: 8 }}>{op.opinion.content}</div>
...
>
  <button
    ...
    style={{
      ...
      background: "#fff",
      fontSize: 12,
      cursor: feedbackLoadingPostId === op.opinion.id ? "default" : "pointer",
    }}
  >
❓ 言葉がわからん
{" "}
...
❓ 反対意見がわからん
{" "}
({op.opinion.feedback_counts?.counterargument_unknown ?? 0})
  </button>
</div>

{op.opinion.prediction_flag && (
  <div
    style={{
      marginBottom: 8,
      padding: "8px 10px",
  ...
  >
    <div style={{ fontWeight: 700, color: "#1565c0" }}>📊 予測投稿</div>
    <div>対象: {op.opinion.prediction_target ?? "未設定"}</div>
    <div>
      期限: {op.opinion.prediction_deadline ? formatDate(op.opinion.prediction_deadline) : "未設定"}
    </div>
    <div>結果: {op.opinion.prediction_result ?? "pending"}</div>
  </div>
)}
...
>
  ...
  >
    <input
      type="checkbox"
      checked={predictionFlag}
      onChange={(e) => setPredictionFlag(e.target.checked)}
      disabled={posting}
    />
    これは予測です
  </label>
    ...
    <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
      <div>
        ...
        <input
          ...
          placeholder="例：2026年夏までに消費税減税が実施される"
          disabled={posting}
          style={{
            width: "100%",
            border: "1px solid #ccc",
